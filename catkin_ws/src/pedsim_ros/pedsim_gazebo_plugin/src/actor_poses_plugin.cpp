...
//                            ROS_INFO_STREAM("actor_id: "<< std::to_string( msg->tracks[actor].track_id) );
                            ...
                            ignition::math::Quaterniond original_orientation(
                                msg->agent_states[actor].pose.orientation.w,
                                msg->agent_states[actor].pose.orientation.x,
                                msg->agent_states[actor].pose.orientation.y,
                                msg->agent_states[actor].pose.orientation.z);

                            // 定義 Z 軸旋轉 90 度（π/2）
                            ignition::math::Quaterniond z_rot(ignition::math::Vector3d(0, 0, IGN_DTOR(90.0)));  // or M_PI/2

                            // 新的旋轉 = 原始旋轉 * Z軸旋轉
                            ignition::math::Quaterniond new_orientation = original_orientation * z_rot;

                            gzb_pose.Rot() = new_orientation;
