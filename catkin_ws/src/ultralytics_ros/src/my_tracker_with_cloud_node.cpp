...
#include "tracker_with_cloud_node/my_tracker_with_cloud_node.h"
...
TrackerWithCloudNode::TrackerWithCloudNode() : pnh_("~")
{
  ...
  detection3d_pub_ = nh_.advertise<ultralytics_ros::Det3DArray>(yolo_3d_result_topic_, 1);
...
void TrackerWithCloudNode::syncCallback(const sensor_msgs::CameraInfo::ConstPtr& camera_info_msg,
                                        ...
                                        const ultralytics_ros::MyYoloResult::ConstPtr& yolo_result_msg)
{
  ...
  transformed_cloud = cloud2TransformedCloud(raw_cloud, cloud_msg->header.frame_id, cam_model_.tfFrame(),
                                             cloud_msg->header.stamp);
  // vision_msgs::Detection3DArray detections3d_msg;
  ultralytics_ros::Det3DArray detections3d_msg;
...
void TrackerWithCloudNode::projectCloud(const pcl::PointCloud<pcl::PointXYZ>::Ptr& cloud,
                                        const ultralytics_ros::MyYoloResultConstPtr& yolo_result_msg,
                                        ...
                                        sensor_msgs::PointCloud2& combine_detection_cloud_msg)
{
  ...
  for (size_t i = 0; i < yolo_result_msg->detections.bboxes.size(); i++)
  {
    pcl::PointCloud<pcl::PointXYZ>::Ptr detection_cloud_raw(new pcl::PointCloud<pcl::PointXYZ>);

    processPointsWithBbox(cloud, yolo_result_msg->detections.bboxes[i], detection_cloud_raw);

    // ROS_INFO("YOLO bbox %d → 3D points: %lu",
    //      yolo_result_msg->detections.bboxes[i].id,
    //      detection_cloud_raw->points.size());
    ...
    if (!detection_cloud_raw->points.empty())
    {
      ...
      createBoundingBox(detections3d_msg, closest_detection_cloud, yolo_result_msg->detections.bboxes[i]);
...
void TrackerWithCloudNode::processPointsWithBbox(const pcl::PointCloud<pcl::PointXYZ>::Ptr& cloud,
                                                 const ultralytics_ros::BBox2D& detection,
                                                 pcl::PointCloud<pcl::PointXYZ>::Ptr& detection_cloud_raw)
{
  for (const auto& point : cloud->points)
  {
    double margin_y = 20; // 垂直方向的邊界擴展，避免點雲過於集中在邊緣
    cv::Point3d pt_cv(point.x, point.y, point.z);
    cv::Point2d uv = cam_model_.project3dToPixel(pt_cv);

    if (point.z <= 0) continue; // 跳過相機後方的點

    // if (point.z > 0 && uv.x > 0 && uv.x >= detection.center.x - detection.size_x / 2 &&
    //     uv.x <= detection.center.x + detection.size_x / 2 &&
    //     uv.y >= detection.center.y - detection.size_y / 2 &&
    //     uv.y <= detection.center.y + detection.size_y / 2)
    if (point.z > 0 && uv.x > 0 && uv.x >= detection.center.x - detection.size_x / 2 &&
        uv.x <= detection.center.x + detection.size_x / 2 &&
        uv.y >= detection.center.y - detection.size_y / 2 && // - margin_y &&
        uv.y <= detection.center.y + detection.size_y / 2 )  // + margin_y)
    {
...
{
  if (cloud->empty()) {
    ROS_INFO("Cluster Bug: Input cloud is empty");
    return cloud; // 返回空聚類
  }
  ...
  if (cluster_indices.empty()) {
    ROS_INFO("Cluster Bug: No cluster found");
  }
...
void TrackerWithCloudNode::createBoundingBox(
    ...
    ultralytics_ros::Det3DArray& detections3d_msg, const pcl::PointCloud<pcl::PointXYZ>::Ptr& cloud,
    const ultralytics_ros::BBox2D& detection)
{
  // vision_msgs::Detection3D detection3d;
  ultralytics_ros::Det3D detection3d;
  pcl::PointCloud<pcl::PointXYZ>::Ptr transformed_cloud(new pcl::PointCloud<pcl::PointXYZ>);
  pcl::PointXYZ min_pt, max_pt;
  Eigen::Vector4f centroid;

  if (cloud->empty()) {
    ROS_INFO("Create BBox Error: Input cloud is empty!");
    return;
  }
  ...
  {
    ROS_INFO("Create BBox Error: Centroid contains NaN or infinite values!");
    return;
  }
  ...
  if (transformed_cloud->empty()) {
    ROS_INFO("Create BBox Error: Transformed cloud is empty!");
    return;
  }
  ...
  detection3d.center.position.x = bbox_center[0];
  detection3d.center.position.y = bbox_center[1];
  detection3d.center.position.z = bbox_center[2];
  detection3d.center.orientation.x = q.x();
  detection3d.center.orientation.y = q.y();
  detection3d.center.orientation.z = q.z();
  detection3d.center.orientation.w = q.w();
  detection3d.size.x = max_pt.x - min_pt.x;
  detection3d.size.y = max_pt.y - min_pt.y;
  detection3d.size.z = max_pt.z - min_pt.z;

  detection3d.score = detection.score;
...
TrackerWithCloudNode::createMarkerArray(const ultralytics_ros::Det3DArray& detections3d_msg, const double& duration)
{
  visualization_msgs::MarkerArray marker_array;
  for (size_t i = 0; i < detections3d_msg.detections.size(); i++)
  {
    if (std::isfinite(detections3d_msg.detections[i].size.x) &&
        std::isfinite(detections3d_msg.detections[i].size.y) &&
        std::isfinite(detections3d_msg.detections[i].size.z))
    {
    //   ROS_INFO("Create marker");
      ...
      marker.pose = detections3d_msg.detections[i].center;
      marker.scale.x = detections3d_msg.detections[i].size.x;
      marker.scale.y = detections3d_msg.detections[i].size.y;
      marker.scale.z = detections3d_msg.detections[i].size.z;
      marker.color.r = 0.0;
      marker.color.g = 1.0;
      marker.color.b = 0.0;
      marker.color.a = 0.5;
    ...
    {
      ROS_INFO("Detection is not finite");
      ROS_INFO("Size x: %f", detections3d_msg.detections[i].size.x);
      ROS_INFO("Size y: %f", detections3d_msg.detections[i].size.y);
      ROS_INFO("Size z: %f", detections3d_msg.detections[i].size.z);
    }
  }
  return marker_array;
}
